#' cross validation function for groupwise sparse SIMPLS
...
#' @export
#' @examples cv.sgSIMPLS <- function(X,Y,npc, n.folds=5, kernel=c("linear","delta"), groups=NULL, nonzero.groups=NULL, alpha=NULL, trace=F,niter=200,parallel=F,n.cores=NULL, ind.names=NULL, part.balance=F)

cv.sgSIMPLS <- function(X,Y,npc, n.folds=5, kernel=c("linear","delta"), groups=NULL, 
                        nonzero.groups=NULL, alpha=NULL, trace=F,niter=200,
                        parallel=F,n.cores=NULL, ind.names=NULL, part.balance=F){
  ...
  if(parallel){ 
    ...
    metric.vec <- parallel::parApply(cl=clust,X=as.matrix(param.grid),1,cv.partition.sparse_group.simpls,df.partition=df.partition,npc=npc,
                                     n.folds=n.folds, groups=groups, kernel=kernel, ind.names=ind.names, niter=niter,trace=trace)
  } else{
    metric.vec <- apply(X=as.matrix(param.grid),1,cv.partition.sparse_group.simpls,df.partition=df.partition,npc=npc,
                        n.folds=n.folds, groups=groups, kernel=kernel, ind.names=ind.names, niter=niter,trace=trace)
  }
  
  param.grid <- cbind(param.grid,metric.vec)
  metric.matrix <- sgmeth::matrix.fill(param.grid=param.grid,n.args=n.args,n.folds=n.folds)
