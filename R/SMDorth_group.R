...
#' @export
#' @examples SMDorth_group(X=Xuse, us=u[,1:(i-1)], d=temp$d, u=temp$u, v=temp$v, n=n, p=p, groups=groups, nonzero.groups=nonzero.groups, unique.group=unique.group, num.group=num.group,niter=niter, trace=trace, npc=i)

SMDorth_group <- function(X, us, d, u, v, n, p, groups, nonzero.groups, unique.group, 
  ...
  for(iter in 1:niter){
    ...
    shrunk.list <- sgmeth::vector.shrinkage(Xtu.list = temp$Xtu.list, Xtu.vec=Xtu.vec, 
                                    Xk.col.vec=Xk.col.vec, lambda=lambda)
