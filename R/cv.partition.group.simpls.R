...
cv.partition.group.simpls <- function(arg.group, df.partition, npc, n.folds, groups, kernel=c("linear", "delta"), ind.names, niter,trace){
  ...
  gr.simpls.out <- sgmeth::gSIMPLS(X=xtrain,Y=ytrain,npc=npc, kernel=kernel, groups=groups, nonzero.groups=nonzero.groups, ind.names=rownames(xtrain), niter=niter, trace=trace)
