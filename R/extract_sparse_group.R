...
#' @param alpha l1 sparse penalty term
...
#' @export
#' @examples extract_sparse_group(X, n, p, groups, nonzero.groups, alpha, unique.group, num.group, niter, trace, npc)

extract_sparse_group <- function(X, n, p, groups, nonzero.groups, alpha, unique.group, num.group, niter, trace, npc){
  temp <- RSpectra::svds(X, k=1)
  return(sgmeth::SMD_sparse_group(X=X,d=temp$d,u=temp$u,v=temp$v,n=n,p=p,
                          groups=groups, nonzero.groups=nonzero.groups, unique.group=unique.group,
                          num.group=num.group, alpha=alpha, niter=niter,trace=trace,npc=npc))
