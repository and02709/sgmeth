...
#' @export
#' @examples SSPCA(X=X,Y=Y,npc=2,sparsity.type="sumabs",nonzero.loadings=NULL,sumabsv=sqrt(10),kernel="linear",niter=200,trace=F)
...
SSPCA <- function(X, Y, npc, sparsity.type=c("loadings", "sumabs"), 
  ...
  if(sparsity.type=="loadings"){
    return(sgmeth::SSPCA.load(X=X,Y=Y,npc=npc,nonzero.loadings = nonzero.loadings,
                             kernel=kernel, niter=niter,trace=trace))
  } 
  if(sparsity.type=="sumabs"){
    return(sgmeth::SSPCA.sumabs(X=X,Y=Y,npc=npc,sumabsv=sumabsv,
                               kernel=kernel, niter=niter,trace=trace))
  }
