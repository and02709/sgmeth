#' This function is the parent function for performing groupwise SIMPLS
...
#' @export
#' @examples gSIMPLS(X,Y,npc, kernel=c("linear","delta"), groups=NULL, nonzero.groups=NULL, ind.names=NULL, niter=50, trace=F)

gSIMPLS <- function(X,Y,npc, kernel=c("linear","delta"), groups=NULL, nonzero.groups=NULL, ind.names=NULL, niter=50, trace=F){
  X <- as.matrix(X)
  ...
  if(is.null(groups) && is.null(nonzero.groups)){
    return(sgmeth::Ssimpls(X=X,Y=Y,npc=npc,nonzero.loadings = p, sparsity.type="loadings",kernel=kernel, niter=niter, trace=trace))
  }
  
  # Perform SSPCA if no groups are specified but values of nonzero.groups is specified
  if(is.null(groups) && !is.null(nonzero.groups)){
    return(sgmeth::Ssimpls(X=X,Y=Y,npc=npc,nonzero.loadings = nonzero.groups, sparsity.type="loadings", kernel=kernel, niter=niter, trace=trace))
  }
  ...
  L <- sgmeth::respkernel(Y=Y, n=n, kernel=kernel)
  # Initialize vector containing vectors
  v <- matrix(0, nrow=p, ncol=npc)
  Xi <- X
  
  for(i in 1:npc){
    Psi <- sgmeth::get.psi(X=Xi, L=L, n=n, p=p)
    udv <- sgmeth::extract_group(X=Psi, n=n, p=p, groups=groups, nonzero.groups=nonzero.groups, unique.group=unique.group, num.group=num.group,
                         niter=niter, trace=trace, npc=i)
    v[,i] <- udv$v
    if(i==npc) break
    Xi <- sgmeth::deflate(X=X,V=v[,1:i],n=n,p=p)
  }
  return(v)
}
