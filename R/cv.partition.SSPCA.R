...
cv.partition.SSPCA <- function(arg.sparse, df.partition, npc, n.folds, sparsity.type=c("loadings", "sumabs"),
  ...
  if(sparsity.type=="loadings") sspca.out <- sgmeth::SSPCA(X=xtrain,Y=ytrain,npc=npc,
                                                          sparsity.type=sparsity.type,nonzero.loadings=nonzero.loadings,
                                                          sumabsv=NULL,kernel=kernel,niter=niter,trace=trace)
  else sspca.out <- sgmeth::SSPCA(X=xtrain,Y=ytrain,npc=npc,
                                 sparsity.type=sparsity.type,nonzero.loadings=NULL,
                                 sumabsv=sumabsv,kernel=kernel,niter=niter,trace=trace)
  V <- sspca.out$v
  
  mod <- sgmeth::model.build.SSPCA(Y=as.matrix(ytrain),X=as.matrix(xtrain),V=V,kernel=kernel)
  metric <- sgmeth::model.metric.SSPCA(Y=as.matrix(ytest),X=as.matrix(xtest),V=V,mod=mod,kernel=kernel)
  return(metric)
}