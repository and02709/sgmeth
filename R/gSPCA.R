#' This function is the parent function for performing groupwise supervised PCA
...
gSPCA <- function(X,Y,npc, kernel=c("linear","delta"), groups=NULL, nonzero.groups=NULL, ind.names=NULL, niter=50, trace=F){
  X <- as.matrix(X)
  ...
  if(is.null(groups) && is.null(nonzero.groups)){
    return(sgmeth::SSPCA(X=X,Y=Y,npc=npc,nonzero.loadings = p, sparsity.type="loadings",kernel=kernel, niter=niter, trace=trace))
  }
  
  # Perform SSPCA if no groups are specified but values of nonzero.groups is specified
  if(is.null(groups) && !is.null(nonzero.groups)){
    return(sgmeth::SSPCA(X=X,Y=Y,npc=npc,nonzero.loadings = nonzero.groups, sparsity.type="loadings", kernel=kernel, niter=niter, trace=trace))
  }
  ...
  Psi <- sgmeth::get.psi(X=X, L=L, n=n, p=p)
  
  # Group decomposition
  out <- sgmeth::PMD_group(X=Psi,npc=npc,n=n,p=p,groups=groups,
                   nonzero.groups=nonzero.groups,unique.group=unique.group,
                   num.group=num.group,niter=niter,trace=trace)
