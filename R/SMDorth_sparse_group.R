...
#' @export
#' @examples SMDorth_sparse_group(X=Xuse, us=u[,1:(i-1)], d=temp$d, u=temp$u, v=temp$v, n=n, p=p, groups=groups, nonzero.groups=nonzero.groups, unique.group=unique.group, alpha=alpha,num.group=num.group, niter=niter, trace=trace, npc=i)

SMDorth_sparse_group <- function(X, us, d, u, v, n, p, groups, nonzero.groups, unique.group, 
  ...
  for(iter in 1:niter){
    ...
    # calculate vector u
    u <- X%*%v
    numer <- lsfit(y=u, x=us, intercept=FALSE)$res
    # normalize vector u
    u <- matrix(matrix(numer/sgmeth::l2n(numer)),ncol=1)
    ...
    lambda <- sgmeth::calculate.sparse.lambda(Xtu.list=temp$Xtu.list, upper.lambda=upper.lambda,
                                      alpha=alpha, nonzero.groups=nonzero.groups)
    
    bin.list <- sgmeth::calculate.shrinkage.binary(Xtu.list=temp$Xtu.list,lambda=lambda,alpha=alpha)
    
    # Shrink each vector by the approprite amount determined by lambda
    shrunk.list <- sgmeth::vector.sparse.shrinkage(Xtu.list=temp$Xtu.list, shrink.list=bin.list, 
