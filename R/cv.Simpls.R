...
cv.Simpls <- function(X, Y ,npc, n.folds, sparsity.type=c("loadings", "sumabs"), 
  ...
  if(parallel){ 
    ...
    metric.vec <- parallel::parApply(cl=clust,X=as.matrix(param.grid),1,cv.partition.Simpls,df.partition=df.partition,npc=npc,
                                     n.folds=n.folds,sparsity.type=sparsity.type,nonzero.loadings=NULL,
                                     sumabsv=NULL,kernel=kernel,niter=niter,trace=trace)
  } else{
    metric.vec <- apply(X=as.matrix(param.grid),1,cv.partition.Simpls,df.partition=df.partition,npc=npc,
                        n.folds=n.folds,sparsity.type=sparsity.type,nonzero.loadings=NULL,
                        sumabsv=NULL,kernel=kernel,niter=niter,trace=trace)
  }
