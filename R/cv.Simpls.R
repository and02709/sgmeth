...
#' @export
#' @examples cv.Simpls <- function(X=X, Y=Y ,npc=2, n.folds=5, sparsity.type="sumabs", nonzero.loadings=NULL, sumabsv=sqrt(10),  kernel="linear", parallel=F, n.cores=NULL, niter=200, trace=F, part.balance=F)

cv.Simpls <- function(X, Y ,npc, n.folds, sparsity.type=c("loadings", "sumabs"), 
                      nonzero.loadings=NULL, sumabsv=NULL, 
                      kernel=c("linear", "delta"), parallel=F, 
                      n.cores=NULL, niter=50, trace=F, part.balance=T,mc.method=T){
  X <- as.matrix(X)
  Y <- as.matrix(Y)
  ...
  if(parallel){ 
    if(mc.method){
      ...
      metric.vec <- unlist(parallel::mclapply(param.grid.l,cv.partition.Simpls,
    ...
    metric.vec <- apply(X=as.matrix(param.grid),1,cv.partition.Simpls,
                        df.partition=df.partition,npc=npc,
