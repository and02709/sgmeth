...
sgSIMPLS <- function(X,Y,npc, kernel=c("linear","delta"), groups=NULL, nonzero.groups=NULL, alpha=0, ind.names=NULL, niter=50, trace=F){
  ...
  if(!is.null(groups) && !is.null(nonzero.groups) && alpha==0){
    return(sgmeth::gSIMPLS(X=X,Y=Y,npc=npc, kernel=kernel, groups=groups, 
  ...
  if(nonzero.groups == num.group){
    ...
    return(sgmeth::Ssimpls(X=X,Y=Y,npc=npc,sumabsv = sparse.penalty, sparsity.type="sumabs", kernel=kernel, niter=niter, trace=trace))
  }
  ...
  for(i in 1:npc){
    ...
    udv <- sgmeth::extract_sparse_group(X=Psi, n=n, p=p, groups=groups, nonzero.groups=nonzero.groups, alpha=alpha, unique.group=unique.group, num.group=num.group,
