...
cv.partition.sparse_group.simpls <- function(arg.group, df.partition, npc, n.folds, groups, kernel=c("linear", "delta"), ind.names, niter,trace){
  ...
  sgr.simpls.out <- sgmeth::sgSIMPLS(X=xtrain,Y=ytrain,npc=npc, kernel=kernel, groups=groups, alpha=alpha, nonzero.groups=nonzero.groups, ind.names=rownames(xtrain), niter=niter, trace=trace)
  
  V <- sgr.simpls.out
  
  mod <- sgmeth::model.build.Simpls(Y=as.matrix(ytrain),X=as.matrix(xtrain),V=V,kernel=kernel)
  metric <- sgmeth::model.metric.Simpls(Y=as.matrix(ytest),X=as.matrix(xtest),V=V,mod=mod,kernel=kernel)
  return(metric)
}