...
sgSPCA <- function(X,Y,npc, kernel=c("linear","delta"), groups=NULL, 
  ...
  if(is.null(groups) && is.null(nonzero.groups)){
    return(sgmeth::SSPCA(X=X,Y=Y,npc=npc,nonzero.loadings=p, sparsity.type="loadings",
                 kernel=kernel, niter=niter, trace=trace))
  }
  
  # Perform SSPCA if no groups are specified but values of nonzero.groups is specified
  if(is.null(groups) && !is.null(nonzero.groups)){
    return(sgmeth::SSPCA(X=X,Y=Y,npc=npc,nonzero.loadings=nonzero.groups, sparsity.type="loadings",
                 kernel=kernel, niter=niter, trace=trace))
  }
  
  if(!is.null(groups) && !is.null(nonzero.groups) && alpha==0){
    return(sgmeth::gSPCA(X=X,Y=Y,npc=npc, kernel=kernel, groups=groups, 
  ...
  if(nonzero.groups == num.group){
    ...
    return(sgmeth::SSPCA(X=X,Y=Y,npc=npc,sumabsv = sparse.penalty, sparsity.type="sumabs", kernel=kernel, niter=niter, trace=trace))
  }
  ...
  # Group decomposition
  out <- sgmeth::sparse.group.PMD(X=Psi,npc=npc,n=n,p=p,groups=groups,
